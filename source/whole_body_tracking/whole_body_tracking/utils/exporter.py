...
def export_motion_policy_as_onnx(
    ...
    filename1 = filename.replace('.onnx', '_action.onnx')
    policy_exporter.export_action(path, filename1)
...
class _OnnxMotionPolicyExporter(_OnnxPolicyExporter):
    ...
    def forward_action(self, x):
        ...
        return actor(self.normalizer(x))
    ...
    class ActionWrapper(nn.Module):
        ...
        def forward(self, x):
            return self.exporter.forward_action(x)
    ...
    def export_action(self, path, filename):
        self.to("cpu")
        obs = torch.zeros(1, self.actor[0].in_features)
        action_wrapper = self.ActionWrapper(self)
        torch.onnx.export(
            action_wrapper,
            (obs,),
            os.path.join(path, filename),
